...
export function processRadioMessage(text) {
    ...
    STATUS_NLP.innerText = "PROCESSING";
    STATUS_NLP.className = "status-badge processing";
    
    // Fake a 1.2s delay for "processing" the language
    setTimeout(() => {
        let lowerText = text.toLowerCase();
        let isCritical = CRITICAL_KEYWORDS.some(kw => lowerText.includes(kw));
        
        if (isCritical) {
            let foundKeywords = CRITICAL_KEYWORDS.filter(kw => lowerText.includes(kw));
