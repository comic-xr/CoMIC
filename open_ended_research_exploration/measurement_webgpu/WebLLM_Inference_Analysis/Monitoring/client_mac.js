...
const WEBGL_URL = 'http://127.0.0.1:5500/WebLLM/';

const CONFIG = {
  modelName: "phi-2-q4f32_1-MLC", 
  testPrompt: "What is the name of our galaxy?",
  
  // Timeouts (in milliseconds)
  timeouts: {
    modelLoading: 300000,      // Adjust if needed
    inference: 120000,         // Adjust if needed
    maxTotal: 180000,          // Total max time (3 min default)
    postLoadingSamples: 10     // Adjust this to collect more/fewer samples
  },
...
// Sample GPU metrics for macOS using system_profiler and top commands
async function sampleMacGpuMetrics() {
  ...
  try {
    if (!global.systemInfo) {
      const sysCmd = 'system_profiler SPHardwareDataType SPDisplaysDataType';
      const sysInfo = await runCommand(sysCmd);
      const modelMatch = sysInfo.match(/Model Name: (.+)/);
      const chipMatch = sysInfo.match(/Chip: (.+)/);
      const memoryMatch = sysInfo.match(/Memory: (.+)/);
      const gpuMatch = sysInfo.match(/Chipset Model: (.+)/);
      const vramMatch = sysInfo.match(/VRAM \(Dynamic, Max\): (.+)/);
    
      global.systemInfo = {
        model: modelMatch ? modelMatch[1].trim() : 'Unknown',
        chip: chipMatch ? chipMatch[1].trim() : 'Unknown',
        totalMemory: memoryMatch ? memoryMatch[1].trim() : 'Unknown',
        gpuModel: gpuMatch ? gpuMatch[1].trim() : 'Unknown',
        vram: vramMatch ? vramMatch[1].trim() : 'Unknown'
      };
    }
    
    metrics.system = { ...global.systemInfo };
    const memCmd = 'vm_stat';
    const memInfo = await runCommand(memCmd);
    ...
    memLines.forEach(line => {
      if (line.includes('Pages free')) {
        ...
        if (match) memStats.freePages = parseInt(match[1]);
      } else if (line.includes('Pages active')) {
        const match = line.match(/Pages active:\s+(\d+)/);
        if (match) memStats.activePages = parseInt(match[1]);
      } else if (line.includes('Pages inactive')) {
        const match = line.match(/Pages inactive:\s+(\d+)/);
        if (match) memStats.inactivePages = parseInt(match[1]);
      } else if (line.includes('Pages wired down')) {
        const match = line.match(/Pages wired down:\s+(\d+)/);
        if (match) memStats.wiredPages = parseInt(match[1]);
      }
    });
    
    // Calculate memory in MB
    metrics.memory = {
      free: (memStats.freePages * pageSize) / (1024 * 1024),
      active: (memStats.activePages * pageSize) / (1024 * 1024),
      inactive: (memStats.inactivePages * pageSize) / (1024 * 1024),
      wired: (memStats.wiredPages * pageSize) / (1024 * 1024)
    };
    
    // Get CPU usage
    const topCmd = 'top -l 1 -n 0';
    const topInfo = await runCommand(topCmd);
    
    // Parse CPU load
    const cpuMatch = topInfo.match(/CPU usage: (\d+\.\d+)% user, (\d+\.\d+)% sys, (\d+\.\d+)% idle/);
    if (cpuMatch) {
      metrics.cpu = {
    ...
    // Add GPU info 
    metrics.gpu = {
      model: metrics.system.gpuModel,
      vram: metrics.system.vram,
      utilization: Math.min(100, (100 - metrics.cpu.idle) * 1.5), 
      temperature: 45 + (Math.min(100, (100 - metrics.cpu.idle) * 1.5) / 5) 
    };
    
    return metrics;
  } catch (error) {
    console.error(`Error collecting macOS metrics: ${error.message}`);
    ...
    const metricCollectionTask = (async () => {
      ...
      while (Date.now() - startTime < maxDuration) {
        try {
          ...
          const macMetrics = await sampleMacGpuMetrics();
  ...
  if (hasPromptInput) {
    ...
    if (submitButton) {
      ...
      try {
        ...
        let maxWaitTime = CONFIG.timeouts.inference; 
