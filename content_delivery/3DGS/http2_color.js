...
function url_worker(self){
    ...
    let root = 'https://localhost:8081/assets/data/'+scene_name+'/w_sort/';
    let all_voxels;
    let downloader;
    ...
    async function fetchDataFromUrl(url, key, priority) {
        ...
        if (downloading_voxels[key]) {
            ...
            downloading_voxels[key]['controller'].abort(); 
            delete downloading_voxels[key];
        }
    ...
    function handleMultipleUrls(urls) {
        Object.entries(urls).forEach(([key, { url, priority }]) => {
            // Pass the url, key, and priority to fetchDataFromUrl
...
function downloadAllPNGs() {
    pngFiles.forEach((dataURL, filename) => {
        const a = document.createElement('a');
        a.href = dataURL;
        a.download = filename;
        a.click();
...
async function main() {
    ...
    const frame = (now) => {
        ...
        if(camera.is_key_frame){
            saveCanvasAsPNG(canvas, scene_name+'_wsort_200_' + camera.img_name)
        }
        */
        render_frame++;
        lastFrame = now;
