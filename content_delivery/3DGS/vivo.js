...
function sequence_worker(self){
    ...
    function calculateVisibilityFactor(vertices, viewProj, viewport) {
        ...
        console.timeEnd('drawface')
    
        console.time('getdata')
        const imageData = context.getImageData(0, 0, width, height).data;
        console.timeEnd('getdata')
        let coveredPixels = 0;
        console.time('getpixels')
        for (let i = 0; i < imageData.length; i += 4) {
            // Check the alpha channel to count covered pixels
            if (imageData[i + 3] > 0) coveredPixels++;
        }
        console.timeEnd('getpixels')
    ...
    function get_voxel_list(viewMatrix,fx,fy,width,height,zfar,viewProj){
        ...
        for(let key_num in this.keys){
            ...
            function getRayDirection(cameraPos, voxelCenter) {
                let dir = [
                    voxelCenter[0] - cameraPos[0],
                    voxelCenter[1] - cameraPos[1],
                    voxelCenter[2] - cameraPos[2]
                ];
                
                let magnitude = Math.sqrt(dir[0] * dir[0] + dir[1] * dir[1] + dir[2] * dir[2]);
                
                if (magnitude === 0) {
                    throw new Error("Camera position and voxel center are the same, no valid ray direction.");
                }
            ...
            function rayBoxIntersect(rayOrigin, rayDir, boxMin, boxMax) {
            ...
            if(pyramidIntersect(tri1, tri2, rect, world_vertices)){
                ...
                let inv4 = invert4(viewMatrix)
                let cam_pose = [inv4[12],inv4[13],inv4[14]]
                let raydir = getRayDirection(cam_pose,center)
                let voxel_distance = l2Distance3D(cam_pose,center)

                let DC;
                if(voxel_distance < 30.2){
                    DC = 5;
                }else if(voxel_distance >= 30.2 && voxel_distance < 40.2){
                    DC = 4;
                }else if(voxel_distance >= 40.2 && voxel_distance < 50.2){
                    DC = 3;
                }else if(voxel_distance >= 50.2 && voxel_distance < 60.2){
                    DC = 2;
                }else if(voxel_distance >= 60.2){
                    DC = 1;
                }
                
                for(let s_voxel of this.keys){
                    if(s_voxel != voxel_name){
                        let s_leftBottom = this.full_dict[s_voxel]['leftBottom'];
                        let s_rightTop = this.full_dict[s_voxel]['rightTop'];
                        let s_center = [(s_leftBottom[0] + s_rightTop[0])/2, (s_leftBottom[1] + s_rightTop[1])/2, (s_leftBottom[2] + s_rightTop[2])/2];
                        let l2 = l2Distance3D(center,s_center)
                        arr_with_index = sorted_array(sc_distance,l2);
                        sc_distance = arr_with_index[0];
                        insert_index = arr_with_index[1];
                        sc.splice(insert_index,0,s_voxel);
                    }
                }
                if(sc.length >= 26){
                    sc_voxel = sc.slice(0, 26);
                }else{
                    sc_voxel = sc;
                }
                
                let occlusion_voxel = []
                for(let sc_v of sc_voxel){
                    let boxmin = this.full_dict[sc_v]['leftBottom']
                    let boxmax = this.full_dict[sc_v]['rightTop']
                    if(rayBoxIntersect(cam_pose,raydir,boxmin,boxmax)){
                        let scv_center = [(boxmin[0] + boxmax[0])/2, (boxmin[1] + boxmax[1])/2, (boxmin[2] + boxmax[2])/2];
                        let scv_distance = l2Distance3D(scv_center,cam_pose)
                        if(scv_distance < voxel_distance){
                            occlusion_voxel.push(sc_v)
                        }
                    }
                }
                if(occlusion_voxel.length == 0){
                    PDL = DC;
                    required_dict[voxel_name]['density'] = PDL;
                    required_dict[voxel_name]['num'] = this.full_dict[voxel_name]['num'];
                }else{
                    let max_amount = 0;
                    for (let oc_v of occlusion_voxel){
                        let gs_num = this.full_dict[oc_v]['num']
                        if(gs_num>max_amount){
                            max_amount = gs_num
                        }
                    }
                    const RC = max_amount / this.full_dict[voxel_name]['num']
                    let OC;
                    const alpha0 = 0.6;
                    const alpha1 = 1.0;
                    const alpha2 = 3.0;
                    const beta = 0.8;
                    const threshold0 = alpha0 * Math.pow(beta, occlusion_voxel.length-1)
                    const threshold1 = alpha1 * Math.pow(beta, occlusion_voxel.length-1)
                    const threshold2 = alpha2 * Math.pow(beta, occlusion_voxel.length-1)

                    if(RC < threshold0){
                        OC = 0;
                    }else if(RC >= threshold0 && RC < threshold1){
                        OC = 1;
                    }else if(RC >= threshold1 && RC < threshold2){
                        OC = 2;
                    }else{
                        OC = 3;
                    }
                    
                    let PDL = Math.max(DC - OC, 0);
                    required_dict[voxel_name]['density'] = PDL;
                    required_dict[voxel_name]['num'] = this.full_dict[voxel_name]['num'];
...
function url_worker(self){
    ...
    function generate_urls(queue, root) {
        ...
        for (let voxel in queue) {
            let density = queue[voxel].density;
            let num = queue[voxel].num;
    ...
    async function fetchDataFromUrl(url, key, density, num) {
        let new_end = Math.floor(num * density * 0.2) * rowLength;
        if (downloaded_voxels.includes(key)) {
            console.log(`Voxel ${key} already fully downloaded.`);
            return;
        }
        ...
        if (downloading_voxels[key]) {
            const current_end = Math.floor(num * downloading_voxels[key]['density'] * 0.2) * rowLength;
            
            console.log(`Current end: ${current_end}, New end: ${new_end}`);
            
            if (downloader[key]["downloadedbytes"] >= new_end) {
                console.log(`Voxel ${key} already has sufficient data downloaded with new density.`);
                downloading_voxels[key]['density'] = density; // Update the density
                delete downloadLocks[key]; // Unlock the voxel after processing
                return;
            }
            if (new_end > current_end) {
                console.log(`Voxel ${key} density changed, pausing current download and resuming with new density.`);
                delete downloading_voxels[key]; // Cancel the current download
            }
        }

        downloading_voxels[key] = { density };

        try {
            console.log(`Starting download for voxel ${key} from byte ${downloader[key]["downloadedbytes"]} to ${new_end - 1}`);
            const response = await fetch(url, {
                mode: "cors",
                credentials: "omit",
                headers: {
                    'Range': `bytes=${downloader[key]["downloadedbytes"]}-${new_end - 1}`,
                }
            });
            ...
            while (true) {
                ...
                if (!downloading_voxels[key]) {
                    console.log(`Download for voxel ${key} paused/canceled.`);
                    reader.cancel();
                    break;
                }
                ...
                if (done) {
                    ...
                    delete downloading_voxels[key]; // Mark as fully downloaded
    ...
    function handleMultipleUrls(urls) {
        Object.entries(urls).forEach(([key, { url, density,number}]) => {
            
            fetchDataFromUrl(url, key, density,number);
        });
    }
        ...
        if(e.data.list){
            ...
            port.onmessage = (event) => {
                if (event.data.required_voxels){
                    required_dict = event.data.required_voxels;
                    ...
                    urls = generate_urls(required_dict, root);
...
async function main() {
    ...
    const frame = (now) => {
        ...
        if(camera.is_key_frame){
            saveCanvasAsPNG(canvas, scene_name+'_vivo_150_' + camera.img_name)
        }
        */
        render_frame++;
