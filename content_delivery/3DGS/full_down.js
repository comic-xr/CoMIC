...
//scene_list = ['bicycle', 'bonsai', 'drjohnson', 'flowers', 'garden','kitchen', 'playroom', 'room', 'stump', 'train', 'treehill','truck']
async function main() {
    ...
    const url = new URL(
        "https://localhost:8081/assets/data/"+scene_name + "/"+ "sh0.ply"
    );
    ...
    const total_vertexcount = req.headers.get("content-length") / rowLength;
    ...
    window.addEventListener("keydown", (e) => {
        ...
        if (/\d/.test(e.key)) {
            ...
            //console.log('world matrix',invert4(viewMatrix));
        }
		if (['-', '_'].includes(e.key)){
    ...
    const frame = (now) => {
        ...
        if(camera.is_key_frame){
            saveCanvasAsPNG(canvas, scene_name+'_fd_200_' + camera.img_name)
        }
        render_frame++;
